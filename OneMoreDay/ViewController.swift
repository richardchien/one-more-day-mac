...
class ViewController: NSViewController {
    ...
    override func viewDidLoad() {
        ...
        var hintText = self.daysView.viewWithTag(2) as NSTextField
        dayText.textColor = color
        hintText.textColor = color
        var colorTitle = NSAttributedString(string: self.formNewHabitBtn.title, attributes: NSDictionary(object: color, forKey: NSForegroundColorAttributeName))
        self.formNewHabitBtn.attributedTitle = colorTitle
    ...
    func loadViewsAfterLayout() {
        ...
        case .FutureOneDay, .Future:
            isAtLaunch ? self.displayGoBtn() : self.displayGoBtnWithoutAnimation()
            isAtLaunch = false
        case .Past, .Same:
            isAtLaunch ? self.displayDaysView() : self.displayDaysViewWithoutAnimation()
            isAtLaunch = false
        default:
            break
        }
    ...
    func displayDaysView() {
        var goBtnRect = goBtnDisplayFrame
        goBtnRect.origin = CGPointMake(goBtnRect.origin.x, -goBtnRect.size.height)
        
        var goBtnDict = NSMutableDictionary()
        goBtnDict.setObject(self.goBtn, forKey: NSViewAnimationTargetKey)
        goBtnDict.setObject(NSValue(rect: self.goBtn.frame), forKey: NSViewAnimationStartFrameKey)
        goBtnDict.setObject(NSValue(rect: goBtnRect), forKey: NSViewAnimationEndFrameKey)
        //goBtnDict.setObject(NSViewAnimationFadeOutEffect, forKey: NSViewAnimationEffectKey)
        var daysViewDict = NSMutableDictionary()
        daysViewDict.setObject(self.daysView, forKey: NSViewAnimationTargetKey)
        daysViewDict.setObject(NSValue(rect: self.daysView.frame), forKey: NSViewAnimationStartFrameKey)
        daysViewDict.setObject(NSValue(rect: daysViewDisplayFrame), forKey: NSViewAnimationEndFrameKey)
        //daysViewDict.setObject(NSViewAnimationFadeInEffect, forKey: NSViewAnimationEffectKey)
        
        let goBtnAnimation = NSViewAnimation(viewAnimations: NSArray(object: goBtnDict))
        let daysViewAnimation = NSViewAnimation(viewAnimations: NSArray(object: daysViewDict))
        goBtnAnimation.duration = 0.3
        daysViewAnimation.duration = 0.3
        goBtnAnimation.startAnimation()
        daysViewAnimation.startAnimation()
    }
    
    func displayDaysViewWithoutAnimation() {
        var goBtnRect = goBtnDisplayFrame
        goBtnRect.origin = CGPointMake(goBtnRect.origin.x, -goBtnRect.size.height)
        self.goBtn.frame = goBtnRect
        self.daysView.frame = daysViewDisplayFrame
    }
    
    func displayGoBtn() {
        var daysViewRect = daysViewDisplayFrame
        daysViewRect.origin = CGPointMake(daysViewRect.origin.x, self.view.frame.size.height)
        
        var goBtnDict = NSMutableDictionary()
        goBtnDict.setObject(self.goBtn, forKey: NSViewAnimationTargetKey)
        goBtnDict.setObject(NSValue(rect: self.goBtn.frame), forKey: NSViewAnimationStartFrameKey)
        goBtnDict.setObject(NSValue(rect: goBtnDisplayFrame), forKey: NSViewAnimationEndFrameKey)
        var daysViewDict = NSMutableDictionary()
        daysViewDict.setObject(self.daysView, forKey: NSViewAnimationTargetKey)
        daysViewDict.setObject(NSValue(rect: self.daysView.frame), forKey: NSViewAnimationStartFrameKey)
        daysViewDict.setObject(NSValue(rect: daysViewRect), forKey: NSViewAnimationEndFrameKey)
    ...
    func displayGoBtnWithoutAnimation() {
        var daysViewRect = daysViewDisplayFrame
        daysViewRect.origin = CGPointMake(daysViewRect.origin.x, self.view.frame.size.height)
        self.goBtn.frame = goBtnDisplayFrame
        self.daysView.frame = daysViewRect
    }
